...
- The parser supports two formats:
  ...
  **New format (recommended):** `<timestamp>,<heap-bytes>`
  - Examples:
    - `1000000,12345678`
    - `1000100,9876543`
    - `1000200,12345.67` (floats allowed)
  - Interpretation:
    - timestamp — numeric value representing the time at which the heap sample was taken
    - heap-bytes — numeric value representing heap size in bytes at that sample
  - GC events are correlated by matching timestamps between phase1 and phase2

  **Old format (legacy):** `<heap-bytes>,<marker-flag>`
  - Examples:
    - `12345678,true`
    - `9876543,false`
  ...
  - Interpretation:
    - heap-bytes — numeric value representing heap size in bytes at that sample
    - marker-flag — `true` or `false` (case-insensitive). A `true` value marks this sample as a GC marker and will be correlated with GC before/after blocks by position.

3) Phase 2 — page dump (GC blocks)
- GC blocks are parsed by searching for header lines.
- The parser supports two formats:

  **New format (recommended):** includes timestamp
  - Pattern: one or more dashes, then `before` or `after`, then `GC <N> @ <timestamp>`, then one or more dashes
  - Examples: `-------before GC 1 @ 1000200 -------` or `---after GC 2 @ 1000500--`
  
  **Old format (legacy):** without timestamp
  - Pattern: one or more dashes, then `before` or `after`, then `GC <N>`, then one or more dashes
  - Examples: `-------before GC 1 -------` or `---after GC 42--`
...
- When phase1 uses the new format (`timestamp,heap-bytes`) and phase2 headers include timestamps (`@ timestamp`), the correlation is done by matching timestamps.
- The parser extracts timestamps from both phase1 samples and phase2 GC block headers.
- A heap sample is marked as a GC event if its timestamp matches a timestamp from any GC block header.
- This provides precise correlation between heap samples and GC events.

**Old format (position-based):**
- When phase1 uses the old format (`heap-bytes,marker`), the loader sorts GC indices and correlates the Nth sorted GC index with the Nth heap marker sample encountered in the heap timeline (if available).
- This is why preserving `true` markers in the heap timeline where the GC dump is taken is important.

6) Minimal valid template examples

**Example 1: New format with timestamps**
```text
phase1: heap use
1000000,10000000
1000100,10120000
...
**Example 2: Old format with markers**
...
- Explanation for Example 1 (new format):
  - Phase1 contains seven samples with timestamps from 1000000 to 1000600.
  - Phase2 contains two GC pairs (GC 1 @ 1000200 and GC 2 @ 1000500) with `before` and `after` sections.
  - The heap samples at timestamps 1000200 and 1000500 (samples 3 and 6) are automatically correlated with the GC events.
  - Each page-type line lists page tokens that will be parsed into arrays of usage objects.

- Explanation for Example 2 (old format):
...
**For new timestamp-based format (recommended):**
- Ensure `phase1` and `phase2` markers are present and spelled similarly (parser uses case-insensitive matching).
- Use consistent timestamp values across phase1 and phase2.
- Include the timestamp in phase2 headers using the `@ timestamp` syntax.
- The timestamps should match between phase1 samples and phase2 GC events for proper correlation.
- Keep page usage tokens simple: only `+`, `-`, and `(NN%)` are parsed.

**For old marker-based format (legacy):**
- Ensure `phase1` and `phase2` markers are present and spelled similarly (parser uses case-insensitive matching).
...
- Malformed heap lines (not two comma-separated values or non-numeric values) → those lines are skipped silently.
