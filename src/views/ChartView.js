// ChartView: Pure view layer for chart rendering and interaction
// Uses ChartViewModel for all data and commands (MVVM pattern)
import { formatSI, formatSeconds } from "../utils/format.js";

export class ChartView {
  constructor(viewModel, container) {
    this.viewModel = viewModel;
    this.model = viewModel.model; // For direct model access where needed
  ...
  _bindHandlers() {
    ...
    this.canvas.addEventListener('mouseleave', () => { this._hoverCandidate = null; this.model._emit('hover', null); });
  ...
  _onMouseMove(ev) {
    ...
    if (plotW <= 0 || plotH <= 0) { this.model._emit('hover', null); return; }
    ...
    if (best.series && best.point && best.d2 < (30 * this.dpr) * (30 * this.dpr)) {
      ...
      this.model._emit('hover', this._hoverCandidate);
    } else {
      this._hoverCandidate = null;
  ...
  _onClick(ev) {
    ...
    if (best.series && best.point && best.d2 < thresh) {
      // Use ViewModel command instead of direct model manipulation
      ...
      // Clear selection using ViewModel
      this.viewModel.clearPinSelection();
    }
  }
  ...
  _onWindowMouseUp(ev) {
    ...
    if (this._boxMode === 'zoom') {
      ...
      if (ev.altKey) {
        ...
        this.model._emit('status', '已向外扩展视窗（Alt 缩小视图）');
      } else {
        ...
        this.model.viewMinX = newMin; this.model.viewMaxX = newMax;
        this.model.resampleInView();
        this.model._emit('status', '已聚焦到所选区域');
      }
      ...
      if (anySelected.length) this.model._emit('status', `已框选 ${anySelected.length} 个标记`);
      else this.model._emit('status', '未选中任何标记');
    }
  ...
  handleKeyEvent(ev) {
    ...
    if (key === 'a') {
      ev.preventDefault();
      this.model.viewMinX = Math.max(0, this.model.viewMinX - panStep);
      this.model.viewMaxX = this.model.viewMinX + span;
      this.model.resampleInView();
    } else if (key === 'd') {
      ev.preventDefault();
      const ext = this.model.computeGlobalExtents();
      this.model.viewMinX = Math.min(Math.max(0, ext.max - span), this.model.viewMinX + panStep);
      this.model.viewMaxX = this.model.viewMinX + span;
      this.model.resampleInView();
    } else if (key === 'w') {
      ev.preventDefault();
      const center = (this.model.viewMinX + this.model.viewMaxX) / 2;
      const newSpan = Math.max(1, span / zoomFactor);
      this.model.viewMinX = Math.max(0, center - newSpan / 2);
      this.model.viewMaxX = this.model.viewMinX + newSpan;
      this.model.resampleInView();
      this.model._emit('status', '已缩小视窗（快捷键）');
    } else if (key === 's') {
      ev.preventDefault();
      const center = (this.model.viewMinX + this.model.viewMaxX) / 2;
      const ext = this.model.computeGlobalExtents();
      ...
      this.model.viewMaxX = this.model.viewMinX + newSpan;
      this.model.resampleInView();
      this.model._emit('status', '已放大视窗（快捷键）');
    } else if (key === 'q') {
      ev.preventDefault();
      if (this._hoverCandidate && this._hoverCandidate.series && this._hoverCandidate.point) {
        ...
        const sel = state.pinnedPoints.filter(p => p.selected);
        if (sel.length > 0) this.viewModel.jumpToPin(sel[0]);
      }
    } else if (key === 'escape') {
      this.viewModel.clearPinSelection();
    } else if (key === 'delete' || key === 'backspace') {
      this.viewModel.deleteSelectedPins();
    }
  }
