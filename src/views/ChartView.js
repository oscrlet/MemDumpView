// ChartView: Pure view layer for chart rendering and interactions
// Depends on ChartViewModel for commands and ChartModel for data (via ViewModel)
...
export class ChartView {
  constructor(viewModel, container) {
    ...
    // Cache of last metrics used by render
    ...
    // Subscribe to model events for re-rendering
    this.model.on('resampled', () => this.render());
    this.model.on('seriesChanged', () => this.render());
    this.model.on('pinnedChanged', () => this.render());
  }
  ...
  render() {
    ...
    const base = this.model.getBasePlotMetrics(this.canvas.width, this.canvas.height, this.dpr);
    const { W, H } = base;
    ...
    // Compute Y label widths for proper margin
    const rows = 5;
    ...
    const margin = {
      left: leftMargin,
      right: base.marginBase.right,
      top: base.marginBase.top,
      bottom: base.marginBase.bottom
    };
    ...
    if (plotW <= 10 || plotH <= 10) {
      ...
      this.model._emit('rendered', { metrics });
      this.ctx.restore();
      return;
    }
    ...
    for (let i = 0; i <= rows; i++) {
      ...
      this.ctx.strokeStyle = '#f2f6fb';
      this.ctx.lineWidth = Math.max(1, this.dpr * 0.5);
      this.ctx.stroke();
      const v = maxY - t * (maxY - minY);
      const label = formatSI(v);
      this.ctx.fillStyle = '#445066';
      this.ctx.fillText(label, margin.left - 8 * this.dpr, y);
    }
    this.ctx.font = `${12 * this.dpr}px sans-serif`;
    ...
    for (let i = 0; i <= cols; i++) {
      ...
      this.ctx.strokeStyle = '#f6f9fc';
      this.ctx.stroke();
      const vSec = minX / 1e6 + t * ((maxX / 1e6) - (minX / 1e6));
      const label = formatSeconds(vSec);
      this.ctx.fillStyle = '#445066';
      this.ctx.fillText(label, x, margin.top + plotH + 8 * this.dpr);
    }
    ...
    // Draw series
    this.ctx.lineWidth = Math.max(1.4 * this.dpr, 1.2);
    ...
    for (const s of this.model.seriesList) {
      ...
      for (let i = 0; i < arr.length; i++) {
        const p = arr[i];
        const px = xToCanvasPx(p[0]), py = yToCanvasPx(p[1]);
        if (!started) {
      ...
      const lx = xToCanvasPx(last[0]), ly = yToCanvasPx(last[1]);
      this.ctx.fillStyle = s.color;
      this.ctx.beginPath();
      this.ctx.arc(lx, ly, Math.max(2.5 * this.dpr, 2), 0, Math.PI * 2);
      this.ctx.fill();
    }
    ...
    for (const p of this.model.pinnedPoints) {
      ...
      const s = this.model.seriesList.find(x => x.id === p.seriesId && x.visible);
      if (!s) continue;
      const px = xToCanvasPx(p.relMicro);
      const py = yToCanvasPx(p.val);
      this.ctx.save();
      ...
      this.ctx.beginPath();
      this.ctx.moveTo(px, margin.top);
      this.ctx.lineTo(px, margin.top + plotH);
      this.ctx.moveTo(margin.left, py);
      this.ctx.lineTo(margin.left + plotW, py);
      this.ctx.stroke();
      this.ctx.setLineDash([]);
      this.ctx.beginPath();
      this.ctx.fillStyle = '#fff';
      this.ctx.lineWidth = Math.max(2, this.dpr * 0.9);
      this.ctx.strokeStyle = p.color || s.color;
      this.ctx.arc(px, py, Math.max(5 * this.dpr, 4), 0, Math.PI * 2);
      this.ctx.fill();
      this.ctx.stroke();
      this.ctx.beginPath();
      this.ctx.fillStyle = p.color || s.color;
      this.ctx.arc(px, py, Math.max(2.5 * this.dpr, 1.5), 0, Math.PI * 2);
      this.ctx.fill();
      if (p.selected) {
        this.ctx.beginPath();
        this.ctx.strokeStyle = 'rgba(43,108,176,0.9)';
        this.ctx.lineWidth = Math.max(1.2, this.dpr);
        this.ctx.arc(px, py, Math.max(8 * this.dpr, 6), 0, Math.PI * 2);
        this.ctx.stroke();
      }
      this.ctx.restore();
    }
    ...
    // Capture metrics for interactions
    const metrics = {
    ...
    this._lastRenderMetrics = metrics;
    this.viewModel.setMetrics(metrics);
    this.model._emit('rendered', { metrics });

    // Update pinned tooltip DOM positions
    this._updatePinnedTooltips(metrics);
  }

  // ========== Pinned Tooltip DOM Management ==========
  _pinnedKey(p) {
    return `${p.seriesId}::${p.relMicro}`;
  }
  ...
  _updatePinnedTooltips(metrics) {
    ...
    if (!this.model.pinnedPoints || this.model.pinnedPoints.length === 0) {
      for (const kv of this.pinnedTooltipMap.entries()) {
        try {
          kv[1].el.remove();
        } catch (e) {}
      }
  ...
  _getClientMetrics() {
    const rect = this.canvas.getBoundingClientRect();
    const metrics = this._lastRenderMetrics || this.model.getBasePlotMetrics(this.canvas.width, this.canvas.height, this.dpr);
    return { rect, metrics };
  }
  ...
  _onMouseMove(ev) {
    ...
    for (const s of this.model.seriesList) {
      ...
      while (hi - lo > 6) {
        const mid = (lo + hi) >> 1;
        if (arr[mid][0] < (minX + (pxDev - metrics.margin.left) / plotW * (maxX - minX))) lo = mid;
        else hi = mid;
      }
    ...
    if (best.series && best.point && best.d2 < (30 * this.dpr) * (30 * this.dpr)) {
      ...
      this.viewModel.setHoverCandidate(this._hoverCandidate);
    } else {
      this._hoverCandidate = null;
      this.viewModel.setHoverCandidate(null);
    }
  }

  _onClick(ev) {
    if (this.model.seriesList.length === 0) return;
    ...
    if (best.series && best.point && best.d2 < thresh) {
      this.viewModel.togglePinAt(
        best.series.id,
        best.point[0],
        best.point[1],
        best.series.color || '#333',
        best.series.name
      );
      this.render();
    } else {
      // Deselect all pins
      for (const p of this.model.pinnedPoints) p.selected = false;
  ...
  _onWindowMouseUp(ev) {
    ...
    if (this._boxMode === 'zoom') {
      ...
      if (ev.altKey) {
        ...
        const currentSpan = Math.max(1, this.model.viewMaxX - this.model.viewMinX || 1);
        ...
        this.model.viewMinX = Math.max(0, newMin);
        this.model.viewMaxX = Math.max(this.model.viewMinX + 1, newMax);
        this.model.resampleInView();
        this.model.setStatus('已向外扩展视窗（Alt 缩小视图）');
      } else {
        // Zoom in
      ...
      for (let i = 0; i < this.model.pinnedPoints.length; i++) {
        const p = this.model.pinnedPoints[i];
      ...
      this.model._emit('pinnedChanged', this.model.pinnedPoints);
      if (anySelected.length) this.model.setStatus(`已框选 ${anySelected.length} 个标记`);
      else this.model.setStatus('未选中任何标记');
    }
  ...
  _onWheel(ev) {
    ...
    this.viewModel.zoom(factor, centerRel);
  }
  ...
  // ========== Touch Handlers (Simplified Stubs) ==========
  _onTouchStart(ev) {
    this._lastTouchTime = Date.now();
    // Touch implementation preserved from original
  }
  ...
  handleKeyEvent(ev) {
    ...
    if (key === 'a') {
      ev.preventDefault();
      this.viewModel.pan(-panStep);
    } else if (key === 'd') {
      ev.preventDefault();
      this.viewModel.pan(panStep);
    } else if (key === 'w') {
      ev.preventDefault();
      this.viewModel.zoom(1 / zoomFactor);
      this.model.setStatus('已缩小视窗（快捷键）');
    } else if (key === 's') {
      ev.preventDefault();
      this.viewModel.zoom(zoomFactor);
      this.model.setStatus('已放大视窗（快捷键）');
    } else if (key === 'q') {
      ev.preventDefault();
      if (this._hoverCandidate && this._hoverCandidate.series && this._hoverCandidate.point) {
        ...
        this.viewModel.togglePinAt(s.id, p[0], p[1], s.color || '#333', s.name);
        this.render();
      } else {
        const sel = this.model.pinnedPoints.filter(p => p.selected);
        if (sel.length > 0) this.viewModel.jumpToPin(sel[0]);
      }
    } else if (key === 'escape') {
      for (const p of this.model.pinnedPoints) p.selected = false;
      this.model._emit('pinnedChanged', this.model.pinnedPoints);
      ...
      const toDel = this.model.pinnedPoints.filter(p => p.selected);
      if (toDel.length === 0) return;
      for (const p of toDel) {
        const idx = this.model.pinnedPoints.indexOf(p);
        if (idx >= 0) {
          const newPins = this.model.pinnedPoints.slice();
